...
> _**Note: Apart from the default bootstrap theme, All the Essential JavaScript components are provided with built-in**_ _responsive_ _**support, so that it will adjust automatically based on the browser viewport.**_
> _**Setting width of any of the Syncfusion components to 100% is simply enough to make it responsive. Including**_ _ej.responsive.css_ _**file will make some components like menu, dialog, schedule to change their appearances accordingly in mobile devices.**_ 
> _**Torender the grid control appropriately in mobile devices,**_ _ejgrid.responsive.css_ _**file needs to be referred separately along with the other css and script file references.**_
