...
The scripts and style sheets that are mandatorily required to render OlapClient widget inside a HTML page are highlighted below in an appropriate order.
...
In-order to initialize a OlapClient widget, first we need to define a “div” tag with an appropriate “id” attribute which acts as a container for OlapClient widget. Then we need to initialize the widget using ejOlapClient method inside “script” tag.
...
{% endhighlight %}
The “url” property in OlapClient widget points the service endpoint where data are processed and fetched in the form of JSON. The service used for the OlapClient widget as endpoint are WCF and WebAPI. Next couple of topics will illustrate the same.

###WebAPI

**Adding a WebAPI Controller**

To add a WebAPI controller in your existing Web Application, right-click on the project in Solution Explorer and select **Add > New Item.** In the **Add New Item** window, select **WebAPI Controller Class** and name it as “OlapClientServiceController.cs”. Finally click **Add.**
...
>**NOTE: While adding WebAPI Controller Class, name it with the suffix “Controller” which is mandatory. For example, in our demo we have named the controller as “OlapClientServiceController”.**
...
Below are the list of namespaces to be added on top of the main class inside `**OlapClientServiceController.cs**` file.
...
Now we need to define the service methods inside OLAPServiceController class, found inside `**OlapClientServiceController.cs**` file, created while adding WebAPI Controller Class to your Web Application.
...
Now, OlapClient will be rendered OlapChart and PivotGird with Customer Count over a period of fiscal  years.
